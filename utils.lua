...
function utils.visualize(im, boxes, scores, thresh, cl_names)
  ...
  local tmp_boxes = torch.zeros(boxes:size(1), 4)
  for i = 1, boxes:size(1) do
      tmp_boxes[i] = boxes[{{i}, {idx[i][1]*4-3, idx[i][1]*4}}]
  end
  boxes = tmp_boxes
...
--  visualize per class
-------------------------------------------------------------------
function utils.visualize_class(im, boxes, scores, thresh, cls_name)
  ...
  local idx_thresh = scores:gt(thresh)
  scores = scores[idx_thresh]
  --idx = idx[idx_thresh]
  ...
  local keep = utils.nms(torch.cat(scores:float(), boxes_thresh:float(),2),0.3)
  
  boxes_thresh = boxes_thresh:index(1,keep)
  scores = scores:index(1,keep)
  --idx = idx:index(1,keep)
  ...
  for i=1,num_boxes do
    ...
    w:setfont(qt.QFont{serif=true,italic=true,size=fontsize,bold=true})
    if cls_name then
      w:show(string.format('%s: %.2f',cls_name,scores[i]))
    else
      w:show(string.format('Unknown: %.2f',scores[i]))
    end
  end
  w:setcolor("red")
  w:setlinewidth(2)
